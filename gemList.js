...
$(function() {
    ...
    window.addEventListener('scroll', function() { //스크롤 맨 밑
    ...
    $(".selection_del").click(function() {
        qVoiceGemList.elDelete();
    }); //선택 삭제 눌렀을 때
    $("#choice").click(function() {
        qVoiceGemList.elSelect(this);
    }); //전체 선택
})
...
function QVoiceGemList(customCfg){
    var gemListCfg = {
        ...
        template    : function(data) {
           return ` <tr>
                   <td><div style="margin-left: 5px"><input type="checkbox" name="chk[]" id="choice_${data.auto_no}" value="${data.auto_no}" class="chk_click"><label for="choice_${data.auto_no}"></label></div></td> 
                   <td>
                       <p style="margin-right: 5px">
                           <span>${(function(ins_date){
                                       if (!ins_date) return '';
        ...
        if(_.size($('.chk_click:checked'))) {
            $.confirmEx('선택한 내역을 삭제 하시겠습니까?', {
                function callback2() {
                    ...
                    if(_.isEmpty(gemListData)) {
                        $("body").append(gemListCfg["noGem"]); 
                        return;
                    } //리스트 데이터 없으면 중지
            ...
            $.alertEx('삭제 하실 내역을 한개이상<br/>선택해주세요');
        }
    },
    ...
    function elSelect(el) {
        $('.chk_click').attr('checked', el.checked);
    }
}
...
QVoiceGemList.prototype.reqSlctDel = function() { //삭제요청
    ...
    this.jsonCall({
        ...
        useMethod:"slctDel",
        slct:__initData.slct,
        logNo: _.map($slctItem, 'value').join(',')
    });
}
